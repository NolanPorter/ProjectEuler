...
public class gridProduct{
	...
	public static void main(String[] args){
		System.out.println("Answer: " + findProd(grid));
	}

	public static int findProd(int[][] grid){
		int prod = 0;
		int maxProd = 0;
		...
		//Diagonal products: top on left, going down to right.
		for(int i = 0; i < 16; i++){
			for(int  j = 0; j < 16; j++){
		...
		//Diagonal products: top on right, going down to left.
		for(int i = 3; i < 20; i++){
			for(int j = 0; j < 16; j++){
				prod = grid[i][j] * grid[i-1][j+1] * grid[i-2][j+2] * grid[i-3][j+3];
				if(prod > maxProd){
